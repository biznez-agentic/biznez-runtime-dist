...
Before implementing, the following must be verified:
...
### 2. Trade-offs of mounting the entire `/etc/nginx`
...
- Runtime config diverges from image config (harder to debug — `docker run` shows different files than the running pod)
...
Add an init container that copies the full nginx configuration tree from the image into the writable emptyDir mount. The main container's existing entrypoint then works unchanged.
...
- `-r` — recursive (copies subdirectories like `templates/`)
...
**Security context:** The init container uses an explicit security context rather than inheriting the frontend's via the helper. This ensures:
...
- If the frontend's security context changes in the future, the init container remains stable

**Why UID 101 can read `/etc/nginx` in the init container:** The Dockerfile's `chown -R nginx:nginx /etc/nginx` (PR #245) grants ownership to UID 101. The init container runs as UID 101, so it can read all files. The destination `/nginx-config` is an emptyDir owned by the pod's fsGroup, writable by any container in the pod.
...
1. **Init container** (`copy-nginx-config`): Reads `/etc/nginx/` from the read-only image layer, copies all contents (including `templates/nginx.conf.template`, `mime.types`, `conf.d/`, etc.) into the writable `nginx-config` emptyDir
2. **Main container** starts with `/etc/nginx` mounted as the writable emptyDir (which now contains all the original nginx config files)
...
### 5. nginx started and serves traffic
...
### 6. CSP headers in response (if applicable)
